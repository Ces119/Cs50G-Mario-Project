...
function LevelMaker.generate(width, height)
    ...
    for x = 1, width do
        ...
        if x >= 2 and x < width -2 and math.random(7) == 1 then
            for y = 7, height do
                table.insert(tiles[y],
                    Tile(x, y, tileID, nil, tileset, topperset))
            end
            ...
            --- Assignment 4 - Stage 2: Key and Lock  
            if math.random(10) == 1 then 
                ...
                -- chance to generate locks

        
                lockPos = math.random(x, width - 10)
                if not lockExits then 
